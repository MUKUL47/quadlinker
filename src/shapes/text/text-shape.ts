import { TextKeyboardEventType } from "../../core/types/type";
...
export abstract class TextShape extends Shape {
  ...
  draw(
    ...
    for (let i = 0; i < this.label.length; i++) {
      ...
      new TextArea(w, h + offsetH - staticOffset, ctx, this.label[i]).draw(
        ctx,
        { boundingBox: false }
      );
    }
  ...
  updateLabel(v: string, type: TextKeyboardEventType): this {
    ...
    if (
      type === "BACKSPACE" &&
      this.label[this.label.length - 1].length === 0 &&
      this.label.length > 1
    ) {
      this.label.pop();
      return this;
    ...
    if (type === "BACKSPACE") {
      this.label[this.label.length - 1] = this.label[
        this.label.length - 1
      ].slice(0, this.label[this.label.length - 1].length - 1);
      return this;
    }

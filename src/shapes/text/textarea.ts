...
export class TextArea extends Shape {
  ...
  constructor(
    ...
    this.vertices = [
      ...
      new Vector2(
        x + textDimens.width + this.offset,
        y +
          textDimens.actualBoundingBoxAscent +
          textDimens.actualBoundingBoxDescent +
          this.offset
      ),
      new Vector2(
        x - this.offset,
  ...
  draw(
    ...
    !!options?.boundingBox &&
      this.boundingBox(ctx, this.getAbsBox(), {
        disableArc: !options?.highlightVertices,
    ...
    ctx.fillText(
      this.label,
      minX + (maxX - minX) / 2 - textDimens.width / 2,
      minY + (maxY - minY) / 2 + textDimens.actualBoundingBoxAscent / 2
    );
  }
  ...
  static getDimensions(
    ...
    return [
      textDimens.width,
      textDimens.actualBoundingBoxAscent - textDimens.actualBoundingBoxDescent,
    ];
  }
}

...
export class RectShape extends Quadrant {
  ...
  get fetchShape() {
    return new Rect(Math.random() * 500, Math.random() * 800, 100).setId(
      PriorityLinker.generateUniqueGlobalId(this.eventListener)
    );
  }
  protected onKeyDown = (args: EventArgsMap["KEY_DOWN"]) => {
    ...
    if (
      (quad &&
        quad instanceof TextShape &&
        args.keyCode >= 32 &&
        args.keyCode <= 126) ||
      args.key === "Backspace" ||
      args.key === "Enter"
    ) {
      if (!["Backspace", "Enter"].includes(args.key) && args.key.length > 1)
        return;
      if (args.key === "Enter") {
